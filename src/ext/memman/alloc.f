...
: scan-list     ( size mem-blk --- mem-blk' t | f )
  ...
  while
    2dup blk.size@          \ get next blocks block size
...
: scan-heaps        ( size ix --- size mem-blk t | size f )
  heaps head@
  begin
    dup heap.bcount @       \ get total number of allocated mem-blks
    #blocks <>              \ are we maxed out on allocations?
    if
      dup>r heap.mapf@      \ no - point to free mem-map
...
: (allocate)        ( size ix --- size mem-blk | f )
  ...
  if
    ...
    dup heap.mapf@          \ get head item of free mem-map
    swap heap.psize @
    ?index >map-n head@
    exit
  then
...
: (alloc)       ( size mem-blk --- addr )
  ...
  dup blk.heap@ >r          \ save this till later

  2dup blk.size@ <>         \ is mem-blk exactly right size or larger
  if                        \ if its larger
    swap split-blk          \ split off the piece we need
    r@ heap.mapf@           \ add add back the bit we didnt need
    swap add-free
  else
    nip
  then

  dup r> heap.mapa@         \ attach newly allocated mem-blk to allocated
  swap add-aloc             \ mem-map

  blk.addr@ meta + ;        \ return address beyond meta data

\ ------------------------------------------------------------------------
\ allocate a buffer but do not erase it

: alloc          ( size --- addr t | f )
  align16 [ meta 2* ]# +    \ set granularity, add space for meta data
