...
\ ------------------------------------------------------------------------
  ...
  42 var max-heaps          \ maximum number of 20 meg heaps

\ a heap structure is 96 bytes in size.  one physical page of memory is 4k
\ we can therefore fit 42 heap structures within a single allocated page.
\ this leaves 64 bytes unused at the end of the page
...
struct: mem-blk             \ this is a descriptor a memory fragment
  lnode: blk.link           \ mem-blks are stored in linked lists
  1 dq blk.addr             \ address of this descriptors buffer
  1 dq blk.size             \ size of this descriptors buffer
  1 dq blk.heap             \ which heap this descriptor belongs to
;struct       \ size now 48 bytes

\ ------------------------------------------------------------------------
\ heap structure

struct: heap
  lnode: heap.link          \ linked list entries
  1 dq heap.pool            \ memory mapping address (pool)
  1 dq heap.psize           \ size of memory mapping
  1 dq heap.blocks          \ pointer to buffer of mem-blks
  1 dq heap.bsize           \ size of mem-blk buffer
  1 dq heap.bcount          \ number of used mem-blk's
  1 dq heap.mapa            \ pointer to mem-map of allocated mem-blk's
  1 dq heap.mapf            \ pointer to mem-map of free mem-blk's
  list: heap.cached         \ a linked list of unassigned mem-blk's
;struct       \ size now 96 bytes

\ ------------------------------------------------------------------------
\ guard blocks at start and end of each allocation

struct: meta
  1 dq meta.mem-blk         \ pointer to this allocations memory block
  1 dq meta.magic           \ indicates whether block is free or allocated
  1 dq meta.pad             \ top secret, you no look!
  1 dq meta.chk             \ xor of all of the above
;struct
