...
: (?largest)        ( largest list --- largest )
  ...
  while
    ...
    blk.size@               \ get size of this item
    umax                    \ get max so far
...
: ?largest        ( heap --- largest )
  heap.mapf@                \ point to heaps free mem-map
...
: .heap-info    ( heap --- )
  ...
  r@ heap.mapf@ ?total 8 .n buffs
  r@ heap.mapa@ ?total 8 .n buffs
  swap 6 .n 6 .n
...
: ?mem-info   ( heap --- largest free alloc fd fa fc ft )
  dup>r ?largest
  r@ heap.mapf@ ?total      buffs
  r@ heap.mapa@ ?total swap buffs

  off> buffs
  r@ heap.cached head@ (?total)
  buffs
