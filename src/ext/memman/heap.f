...
: (creat-heap)  ( psize pool bsize blks heap --- )
  ...
  dup r@ heap.mapa! mem-map +  \ put free mem-map array in blocks buffer
  dup r@ heap.mapf! mem-map +  \ put allocated mem-map array in blocks buffer

  r@ heap.blocks !          \ set address of mem-blk cache
  r@ heap.bsize !           \ save size of mem-blk buffer

  ( psize pool --- )

  2dup
  r@ heap.pool !            \ store size of and address of pool in heap
  r@ heap.psize !           \ remember size of this mapping

  swap r@ describe          \ create descriptor for entire heap pool
  ( mem-blk --- )

  r@ heap.mapf@ swap add-free  \ add single mem-blk descriptor for entire
...
: destry-heap   ( heap --- )
  ...
  heap.bsize @ r@           \ deallocate mem-blk buffer associated with
  heap.blocks @             \ get address of mem-blk descriptor buffer
  [ mem-map 2* ]# -         \ mapped buffer is 2 mem-maps lower in mem
  <munmap>

  r@ heap.psize @           \ deallocate heap pool buffer
  r@ heap.pool @ <munmap>
