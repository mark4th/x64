...
: wemit         ( c1 win --- )
  over
  case:
    $0d of .cr                  \ emit a cr to the window
    $0a of 2drop                \ ignore this char
    default: .c                 \ default: write char advance cursor
  ;case ;
...
: .win-row      ( row win --- )
  ...
  r@ win-xco@ 4* +          \ plus window x coordinate = destination addr
