...
struct: llist
  1 dq ll.head              \ pointer to head node of list
  1 dq ll.tail              \ pointer to tail node of list
;struct
...
struct: llnode
  1 dq ln.next              \ pointer to next node in list
  1 dq ln.prev              \ pointer to previous node in list
  1 dq ln.list              \ pointer to nodes parent list structure
;struct
...
: <list     ( node --- node )
  ?<head ?exit              \ removeing head or tail node?
  ?<tail ?exit
