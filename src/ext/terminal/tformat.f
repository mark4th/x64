...
: format      ( n1 n2 --- )
  create w, w,
  does>       ( ... --- )
    wcount                  \ fetch parameter count for format string
    swap w@ t-strings + w@  \ get format string offset in t-table
...
: vpa   ( n --- )   dup !> #line vpa ;
: cr    ( --- )     cr off> #out #line 1+ rows min !> #line ;
