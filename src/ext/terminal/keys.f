...
\ ------------------------------------------------------------------------
\ keyboard escape sequences - these are input sequences not output
...
\ executing the above words compiles the key input sequence into the
\ $buffer output buffer but were not going to be outputting it
\ we compare the compiled string against the sequences of characters
\ that were input when we hit a key.
