...
: #!
  on> ?#!                   \ shebang line must contain a -sfload
...
: ?shebang
  ?#! not ?exit             \ if we ran from a #! script then
...
: next-arg
  ...
  case:
    0 of arg-h              \ unknown arg
    1 of do-f               \ fload specified file
    2 of do-s               \ fload a shebanged script
    default: arg-h          \ display useage info
  ;case ;

\ ------------------------------------------------------------------------

: (doargs)
  off> ?#!                  \ assume not running from #! script
